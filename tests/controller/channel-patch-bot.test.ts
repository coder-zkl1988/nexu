...
function makeTempDir(): string {
  const dir = resolve(
    tmpdir(),
    `nexu-channel-patch-bot-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
...
describe("ChannelService.updateChannelBot (service-level)", () => {
  let tempDir: string;
  ...
  it("updates botId and triggers sync", async () => {
    const env = createEnv(tempDir);
    const store = new NexuConfigStore(env);

    const botA = await store.createBot({ name: "Bot A", slug: "bot-a" });
    const botB = await store.createBot({ name: "Bot B", slug: "bot-b" });
    ...
    const updated = await service.updateChannelBot(channel.id, botB.id);

    expect(updated.botId).toBe(botB.id);
    expect(updated.id).toBe(channel.id);
    expect(syncAll).toHaveBeenCalledTimes(1);

    const reread = await store.getChannel(channel.id);
    expect(reread?.botId).toBe(botB.id);
  });

  it("throws when botId is unknown", async () => {
    const env = createEnv(tempDir);
    const store = new NexuConfigStore(env);
    ...
    const { service, syncAll } = makeService(env, store);
    await expect(
      service.updateChannelBot(channel.id, "bot_does_not_exist"),
  ...
  it("throws when channelId is unknown", async () => {
    ...
    const { service, syncAll } = makeService(env, store);
    await expect(
      service.updateChannelBot("channel_does_not_exist", bot.id),
...
function createHttpTestContainer(opts: {
  updateChannelBot?: ReturnType<typeof vi.fn>;
}): ControllerContainer {
  const env = createEnv("/tmp/nexu-channel-patch-bot-http");
  const channelService = {
    listChannels: vi.fn().mockResolvedValue([]),
    getChannel: vi.fn().mockResolvedValue(null),
    updateChannelBot:
      opts.updateChannelBot ?? vi.fn().mockResolvedValue(makeChannelRecord()),
...
describe("PATCH /api/v1/channels/:id (HTTP)", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });

  it("returns 200 and updated channel on success", async () => {
    const updated = makeChannelRecord({ botId: "bot-b" });
    const updateFn = vi.fn().mockResolvedValue(updated);
    const container = createHttpTestContainer({ updateChannelBot: updateFn });
    const app = createApp(container);

    const resp = await app.request("/api/v1/channels/ch-1", {
      method: "PATCH",
    ...
    expect(body).toMatchObject({ id: "ch-1", botId: "bot-b" });
    expect(updateFn).toHaveBeenCalledWith("ch-1", "bot-b");
  });

  it("returns 400 when botId is invalid", async () => {
    const updateFn = vi
      .fn()
      .mockRejectedValue(new Error("Bot not found: bot-missing"));
    const container = createHttpTestContainer({ updateChannelBot: updateFn });
    const app = createApp(container);
    ...
    const resp = await app.request("/api/v1/channels/ch-1", {
      method: "PATCH",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ botId: "bot-missing" }),
    });
  ...
  it("returns 404 when channelId is unknown", async () => {
    ...
    const resp = await app.request("/api/v1/channels/ch-nope", {
      method: "PATCH",
      headers: { "content-type": "application/json" },
