...
- The renderer executes browser commands because it owns the panel that gives the view its bounds. A command that arrives with no panel open fails with an explanation rather than driving a view nobody can see, so "the user can watch the agent work" is structural rather than conventional.
...
- Non-sandbox OpenClaw runs compile with `tools.exec.security="deny"` and deny the `exec`/`process` tools, regardless of whether browser control or Computer Use is enabled. Host shell execution is not an implicit local-automation backend; restoring it requires a separate capability, approval contract, and audit boundary.
