...
- Local automation lifecycle operations are serialized. A completed disable operation must leave both persisted capability state and its platform daemon disabled, even when pairing or permission requests arrive concurrently.
- Browser pairing output and command failures are secret-bearing. API errors must use fixed redacted messages and must never include command stdout or stderr.
- Computer Use is constrained by both the compiled MCP include list and a runtime plugin allowlist. Unknown Peekaboo/CUA tools fail closed; Peekaboo's nested `agent` and remote-debugging `browser` tools are not valid substitutes for Nexu's reviewed Computer Use surface or extension-paired browser flow.
- Non-sandbox OpenClaw runs compile with `tools.exec.security="deny"` and deny the `exec`/`process` tools, regardless of whether Browser or Computer Use is enabled. Host shell execution is not an implicit local-automation backend; restoring it requires a separate capability, approval contract, and audit boundary.
