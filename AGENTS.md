...
- Agent browser control does not survive a controller restart: the desktop relay stops receiving commands and every browser tool answers `no desktop browser panel is listening`, which usually makes the model fall back to Computer Use. Recover with `pnpm dev stop` -> kill any process still listening on the controller port -> `pnpm dev start`. Restarting only the desktop is often not enough, because `pnpm dev` can lose track of the controller and leave an orphan: `pnpm dev status controller` reports `stopped` while that pid still serves `/health`, and the launcher then refuses to start the desktop at all. Unresolved; see the known-issue section in `specs/design-docs/2026-07-27-computer-browser-control.md`, and `<userData>/logs/agent-browser.log` for the relay's connect/end/fail trail.
