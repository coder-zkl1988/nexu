...
> 状态：设计稿，未动代码。拍板记录（2026-09-04）：评论**现在立项**，排 P3 首位，**先设计后实现**；运营文档原话「评论需先设计内容审核与风控」。脑图给评论的权重是 3%，即它是浏览之外的点缀，不是主线。
...
- 帖子摘要靠手机回传（a11y 文本）还是截图 OCR？倾向 a11y 文本（已注入页面结构，成本低），视频帖无正文时跳过。
- 候选生成用哪个模型：与资料生成同一 `generateText` 通道即可；如拒绝率高再换。
- 帖子搜不到（被删/改标题）的比例若高，需要手机在浏览时回传帖子链接或 noteId 以便直达——涉及分享→复制链接，有风控成本，先观察再定。
- 被限流后的恢复策略（停几天、是否清空 pending）——灰度后定。
